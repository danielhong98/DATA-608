...
con_df <- tsdf(con)
con_df <- data.frame(
  x = as.numeric(time(con)),
  y = as.numeric(con)
)

p <- ggplot(con_df, aes(x = x, y = y)) + geom_line(colour = "black") + labs(x = "Year", y = "Spend $M", title = "Total Construction Spending")
p + geom_hline(yintercept = 71540)
...
p1 <- ggplot(df, aes(V1, V2, group = 1)) + geom_bar(stat = "identity") + labs(x = "Year", y = "Spend $M", title = "Total Construction Spending")
p1
```

Summary Time Series Plots for Total Construction Spending Data
```{r}
autoplot(stl(con, s.window = "periodic"), ts.colour = 'blue')
autoplot(acf(con, plot = FALSE))
autoplot(acf(con, plot = FALSE), conf.int.fill = '#0000FF', conf.int.value = 0.8, conf.int.type = 'ma')
autoplot(spec.ar(con, plot = FALSE))
ggcpgram(arima.sim(list(ar = c(0.7, -0.5)), n = 50))
ggtsdiag(auto.arima(con))
gglagplot(con, lags = 4)
ggfreqplot(con)
ggfreqplot(con, freq = 4)
```

Decomposition of the time series shows the observed data broken down by random, seasonal and trend. Seasonal adjustment is made and unit root test is performed. Plot compares seasonally adjusted construction spend to raw data.
```{r}
ggplot(con_df, aes(x = x, y = y)) +
  geom_line(colour = "grey80") +
  stat_seas(start = c(1993, 1), frequency = 12) +
  ggtitle("Construction Spend Seasonal Adjustment") +
  ylab("Construction Spend Per Month")
```
...
autoplot(diff(con_sa))
autoplot(acf(diff(log(con_sa))))
autoplot(pacf(diff(log(con_sa))))
```

Dickey-Fuller test of the null hypothesis to see whether a unit root is present in an autoregressive model.
```{r}
test <- ur.df(con,type=c("trend"),lags=3,selectlags="AIC")
summary(test)
```
...
autoplot(con,main= "Total Construction Spending")
lncon<-log(con)
autoplot(lncon,main= "Log Total Construction Spending")
```
...
autoplot(acf(residuals))
autoplot(pacf(residuals))
...
qplot(residuals, geom = "histogram", main="Residual Histogram", ylab="frequency density")
```
